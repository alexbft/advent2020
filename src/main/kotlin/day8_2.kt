package my.advent2020.day8part2

import java.io.File

enum class Command {
    Jmp, Acc, Nop
}

fun day8_2() {
    data class Instruction(val cmd: Command, val arg: Int)

    val instructions = mutableListOf<Instruction>()
    File("8.txt").useLines { lines ->
        val re = """(\w+) ([+-])(\d+)""".toRegex()
        for (line in lines) {
            val match = re.matchEntire(line)!!
            val cmd = when (match.groupValues[1]) {
                "acc" -> Command.Acc
                "jmp" -> Command.Jmp
                "nop" -> Command.Nop
                else -> throw Exception()
            }
            var arg = match.groupValues[3].toInt()
            if (match.groupValues[2] == "-") {
                arg = -arg
            }
            instructions.add(Instruction(cmd, arg))
        }
    }
    var result = 0
    var terminatedNormally = false
    for (change in instructions.indices) {
        val changedInstruction = instructions[change]
        if (changedInstruction.cmd !in setOf(Command.Jmp, Command.Nop)) {
            continue
        }
        val newCommand = if (changedInstruction.cmd == Command.Nop) Command.Jmp else Command.Nop

        var iptr = 0
        var reg = 0
        val visited = mutableSetOf<Int>()
        while (iptr !in visited) {
            visited.add(iptr)
            if (iptr !in instructions.indices) {
                if (iptr == instructions.size) {
                    terminatedNormally = true
                    result = reg
                }
                break
            }
            val instruction = instructions[iptr]
            val cmd = if (iptr == change) newCommand else instruction.cmd
            when (cmd) {
                Command.Nop -> {
                    iptr++
                }

                Command.Acc -> {
                    reg += instruction.arg
                    iptr++
                }

                Command.Jmp -> {
                    iptr += instruction.arg
                }
            }
        }
        if (terminatedNormally) {
            break
        }
    }
    if (terminatedNormally) {
        println(result)
    } else {
        println("No solution")
    }
}
